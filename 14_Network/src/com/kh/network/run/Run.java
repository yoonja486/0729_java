...
public class Run {
	...
	public static void main(String[] args) {
		 ...
		 * 네트워크의 목적 : 데이터의 교환
		 * 
		 * 통신이 가능한 장치는 일반적으로 Host라고 통칭하며, 종단간의 실제 통신 장치는 EndPoint라고 표현함
		 * 
		 * MAC(Media Access Contorler) Address / IP(Internet Protocol) Address
		 * 
		 ...
		 * 	  단, 값을 바꿀 때 IP주소는 같은 네트워크 범위 안에서, Port번호는 컴퓨터 내부에서 중복이 발생되어선 안됨! 충돌이 일어남!!
		 * 
		 * - Server와 Client
		 * 
		...
		// InetAddress : 네트워크 정보를 활인할 수 있는 클래스
		...
		try {
			...
			// DomainName : 사용자가 서비스를 이용할 때 서버의 IP주소를 외우기 어려우니깐 외우기 쉬운 구조로 사용자에게 제공되는 이름
